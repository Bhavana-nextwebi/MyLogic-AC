...
public partial class Admin_view_admin_users : System.Web.UI.Page
{
    ...
    public void GetAllUsers()
    {
        ...
        {
            strUsers = "";
            List<CreateUser> instrs = CreateUser.GetAllUser(conML);
            int i = 0;
...
<div class='form-check form-switch markBlocked' id='chk_" + cat.Id + @"'  data-id='" + cat.Id + @"'>
  ...
  <td>" + chk + @"</td> 
    ...
    public static string Delete(string id)
    {
        ...
        {
            SqlConnection conML = new SqlConnection(ConfigurationManager.ConnectionStrings["conML"].ConnectionString);
            if (CreateUser.CheckAccess(conML, "view-admin-users.aspx", "Delete", HttpContext.Current.Request.Cookies["ml_aid"].Value))
            {
                ...
                cat.AddedBy = HttpContext.Current.Request.Cookies["ml_aid"].Value;
                int exec = CreateUser.DeleteUser(conML, cat);
                if (exec > 0)
                {
                    x = "Success";
                }
    ...
    public static string BlockUsers(string id, string ftr)
    {
        ...
        {
            SqlConnection conML = new SqlConnection(ConfigurationManager.ConnectionStrings["conML"].ConnectionString);
            if (CreateUser.CheckAccess(conML, "view-admin-users.aspx", "Edit", HttpContext.Current.Request.Cookies["ml_aid"].Value))
            {
                int exec = 0;
                CreateUser pro = new CreateUser();
                if (ftr == "Yes")
                {
                    exec = CreateUser.BlockUser(conML, id, HttpContext.Current.Request.Cookies["ml_aid"].Value);
                    vali = "Blocked";
                }
                else
                {
                    exec = CreateUser.UnBlockUser(conML, id, HttpContext.Current.Request.Cookies["ml_aid"].Value);
                    vali = "UnBlocked";
                }
