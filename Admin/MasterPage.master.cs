...
{
    SqlConnection conML = new SqlConnection(ConfigurationManager.ConnectionStrings["conML"].ToString());
      ...
      public void CheckPassKeyChanged()
    {
        try
        {
            if (Request.Cookies["ml_apkv"] != null)
            {
                string stored_pass_key = Request.Cookies["ml_apkv"].Value;
                var udetails = CreateUser.GetUserDetails(conML, Request.Cookies["ml_aid"].Value);
                if (udetails != null)
                {
                    ...
                    if (current_pass_key != stored_pass_key)
                    {
                        ...
                        if (Request.Cookies["ml_aid"] != null)
                        {
                            Response.Cookies["ml_aid"].Expires = TimeStamps.UTCTime().AddDays(-10);
                        }
                        if (Request.Cookies["ml_apkv"] != null)
                        {
                            Response.Cookies["ml_apkv"].Expires = TimeStamps.UTCTime().AddDays(-10);
                        }
                        Response.Redirect("Default.aspx");
                    }
                }
    ...
    protected void btnLock_Click(object sender, EventArgs e)
    {
        try
        {
            if (Request.Cookies["ml_aid"] != null)
            {
                string uid = Request.Cookies["ml_aid"].Value;
                Response.Cookies["ml_aid"].Expires = TimeStamps.UTCTime().AddDays(-10);
    ...
    public void BindMenuLink()
    {
        try
        {
            ...
            if (CreateUser.CheckIfAdmin(conML, Request.Cookies["ml_aid"].Value) == true)
            {
                strDashLink = "dashboard.aspx";
                List<UserAccess> ua = CreateUser.LinkIfAdmin(conML).OrderBy(x => x.GroupOrder).ToList();
            ...
            {
                List<UserAccess> ua = CreateUser.GetAllUserAccess(conML, Request.Cookies["ml_aid"].Value).OrderBy(x => x.GroupOrder).ToList();
