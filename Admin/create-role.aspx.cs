...
public partial class Admin_create_role : System.Web.UI.Page
{
            ...
            if (Page.IsValid)
            {
                ...
                if (btnSave.Text == "Update")
                {
                    if (CreateUser.CheckAccess(conML, pageName, "Edit", Request.Cookies["ml_aid"].Value))
                    {
                        ...
                        ca.Status = "Active";
                        int result = UserRoles.UpdateUserRoles(conML, ca);
                        if (result > 0)
                        {
                            GetAllRoles();
                            GetRoles();
                ...
                {
                    if (CreateUser.CheckAccess(conML, pageName, "Add", Request.Cookies["ml_aid"].Value))
                    {
                        ca.RoleName = txtName.Text;
                        ca.UpdatedOn = TimeStamps.UTCTime();
                        ca.Status = "Active";
                        int result = UserRoles.InsertUserRoles(conML, ca);
                        if (result > 0)
                        {
                            txtName.Text = "";
                            GetAllRoles();
    ...
    public void GetAllRoles()
    {
        ...
        {
            strUserRoless = "";
            List<UserRoles> cas = UserRoles.GetAllUserRoles(conML).OrderByDescending(s => Convert.ToDateTime(s.UpdatedOn)).ToList();
            int i = 0;
...
<td>" + ca.UpdatedOn.ToString("dd-MMM-yyyy") + @"</td>  
    ...
    public static string Delete(string id)
    {
        ...
        {
            SqlConnection conML = new SqlConnection(ConfigurationManager.ConnectionStrings["conML"].ConnectionString);
            if (CreateUser.CheckAccess(conML, "create-role.aspx", "Delete", HttpContext.Current.Request.Cookies["ml_aid"].Value))
            {
                UserRoles cat = new UserRoles();
                cat.Id = Convert.ToInt32(id);
                cat.UpdatedOn = TimeStamps.UTCTime();
                int exec = UserRoles.DeleteUserRoles(conML, cat);
                if (exec > 0)
                {
                    x = "Success";
                }
