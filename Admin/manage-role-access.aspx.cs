...
public partial class Admin_manage_role_access : System.Web.UI.Page
{
    ...
    public void GetRoleDetails()
    {
        try
        {
            List<UserRoles> comps = UserRoles.GetAllUserRoles(conML).Where(x => x.Id == Convert.ToInt32(Request.QueryString["id"])).ToList();
            if (comps.Count > 0)
            {
    ...
    public void GetUserAccess()
    {
        try
        {
            List<UserRolesAccess> users = UserRoles.GetAllUserRoles(conML, Request.QueryString["id"]).OrderBy(x => x.GOrderBy).ToList();
            int i = 0;
...
<div class='form-check form-switch changeAccess' id='v_" + rl.PageId + @"'  data-id='" + rl.PageId + @"'>
    ...
    public static string ChangeStatus(string pid, string rid, string v, string a, string e, string d)
    {
        ...
        try
        {
            ...
            SqlConnection conML = new SqlConnection(ConfigurationManager.ConnectionStrings["conML"].ConnectionString);
            List<UserRolesAccess> users = UserRoles.GetAllUserRolesCheck(conML, rid, pid);
            UserRolesAccess ua = new UserRolesAccess();
            if (users.Count > 0)
            {
                if (CreateUser.CheckAccess(conML, "manage-role-access.aspx", "Edit", HttpContext.Current.Request.Cookies["ml_aid"].Value))
                {
                    ua.PageId = pid;
                    ua.RoleId = rid;
                    ...
                    int exec = UserRoles.UpdateRoleAccess(conML, ua);
                    if (exec > 0)
                    {
                        x = "Success";
                    }
            ...
            {
                if (CreateUser.CheckAccess(conML, "manage-role-access.aspx", "Add", HttpContext.Current.Request.Cookies["ml_aid"].Value))
                {
                    ua.PageId = pid;
                    ua.RoleId = rid;
                    ua.AddAccess = a == "Y" ? "1" : "0";
                    ...
                    ua.Status = "Active";
                    ua.PageGroupId = "";
                    int exec = UserRoles.InsertToRoleAccess(conML, ua);
                    if (exec > 0)
                    {
                        x = "Success";
                    }
