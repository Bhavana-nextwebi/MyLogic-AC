...
public partial class Admin_add_blog : System.Web.UI.Page
{
    ...
    public void GetBlogs()
    {
        try
        {
            BlogDetails BD = BlogDetails.GetAllBlogDetailsWithId(conML, Convert.ToInt32(Request.QueryString["id"])).FirstOrDefault();
            if (BD != null)
            {
                divimg.Visible = true;
                ...
                txtPosteddate.Text = Convert.ToDateTime(BD.PostedOn).ToString("dd/MMM/yyyy");
                if (BD.ThumbImage != "")
                {
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        try
        {
            if (Page.IsValid)
            {
                ...
                var aid = Request.Cookies["ml_aid"].Value;
                ...
                BD.PostedOn = txtPosteddate.Text.ToString();
                BD.AddedBy = aid;
                ...
                if (BD.ThumbImage != "" && BD.BlogImage != "")
                {
                    if (btnSave.Text == "Update")
                    {
                        ...
                        int result = BlogDetails.UpdateBlogDetails(conML, BD);
                        if (result > 0)
                        {
                    ...
                    {
                        int result = BlogDetails.InsertBlogDetails(conML, BD);
                        if (result > 0)
                        {
                            ...
                            txtDesc.Text = txtMetaDesc.Text = txtName.Text = txtUrl.Text = txtPageTitle.Text = txtMetaKey.Text = txtPostedBy.Text = "";
        ...
        if (Thumbimage.HasFile)
        {
            try
            {
                ...
                if ((fileExtension == ".jpg" || fileExtension == ".jpeg" || fileExtension == ".png" || fileExtension == ".gif" || fileExtension == ".webp"))
                {
                    ...
                    if ((bitimg.PhysicalDimension.Height != 781) || (bitimg.PhysicalDimension.Width != 1050))
                    {
                        return "Size";
                    }
                }
        ...
        if (BlogImage.HasFile)
        {
            try
            {
                ...
                if ((fileExtension == ".jpg" || fileExtension == ".jpeg" || fileExtension == ".png" || fileExtension == ".gif" || fileExtension == ".webp"))
                {
                    ...
                    if ((bitimg.PhysicalDimension.Height != 510) || (bitimg.PhysicalDimension.Width != 1050))
                    {
                        return "Size";
                    }
                }
