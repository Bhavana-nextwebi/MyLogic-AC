        ...
        try
        {
            ...
            if (Page.IsValid)
            {
                if (CreateUser.CheckAccess(conML, pageName, "Edit", Request.Cookies["ml_aid"].Value))
                {
                    ...
                    inputs.UserId = Request.Cookies["ml_aid"].Value;
                    inputs.Password = CommonModel.Encrypt(txtCurrent.Text.Trim());
                    CreateUser logins = CreateUser.Login2(conML, inputs);
                    if (logins.UserGuid != null)
                    {
                        if (logins.Status == "Active")
                        {
                            string status = CreateUser.ChangePassword(conML, logins.UserGuid, CommonModel.Encrypt(txtNew.Text.Trim()));
                            if (status == "Success")
                            {
                                ClearAllUserSession();
